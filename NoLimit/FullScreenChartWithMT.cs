...
{
    public partial class FullScreenChartWithMT : Form
    {

        private const float CZoomScale = 4f;
        private int FZoomLevel = 0;

        public FullScreenChartWithMT()
        {
            InitializeComponent();
        ...
        private void FullScreenChartWithMT_Load(object sender, EventArgs e)
        {
            ...
            while (dr2.Read())
            {
                ...
                comboBox2.Items.Add(tableName.Substring(2));
            }
            dr2.Close();
            //cmd3 = new SqlCommand("SELECT scrip,in_portfolio,price FROM fundamental WHERE in_portfolio=1 ORDER BY scrip", con);
            cmd3 = new SqlCommand("SELECT scrip FROM fundamental WHERE  derivative='Y' ORDER BY scrip", con);
            ...
            while (dr3.Read())
            {
                ...
                comboBox3.Items.Add(tableName.Substring(2));
            }
            dr3.Close();
            cmd4 = new SqlCommand("SELECT scrip FROM fundamental WHERE derivative='N' OR derivative='E' ORDER BY scrip", con);
            dr4 = cmd4.ExecuteReader();
            while (dr4.Read())
            {
                string tableName = dr4[0].ToString();
                comboBox4.Items.Add(tableName.Substring(2));
            }
            dr4.Close();
        ...
        {
            chtEquity.Series.Clear();
            PopulateChartFromDB(comboBox4.SelectedItem.ToString()); 
            System.Diagnostics.Process.Start("https://www1.nseindia.com/marketinfo/companyTracker/resultsCompare.jsp?symbol=" + comboBox4.SelectedItem.ToString().ToUpper());
        }
